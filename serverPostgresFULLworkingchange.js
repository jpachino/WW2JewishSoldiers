...
//const multiUpload = upload.array('m_files[]', 12);
...
app.post('/addFULL', multiUpload, async (req, res) => {
    try {
        ...
        await db.tx(async t => {
            // 1. Insert Soldier and get ID
            const query = pgp.helpers.insert(cleaned, csSoldiers) + ' RETURNING id';
            const { id: soldierId } = await t.one(query);

            // 2. Multimedia Setup
            const mTypes = [].concat(req.body['m_type[]'] || []);
            const mDescs = [].concat(req.body['m_description[]'] || []);
            const mUrls = [].concat(req.body['physical_logical_location[]'] || []);
            const files = req.files || [];
            
            let filePointer = 0; 
            let urlPointer = 0; // Added to track URL array separately
            ...
            for (let i = 0; i < mTypes.length; i++) {
                let finalPath = '';
                const currentType = mTypes[i];
                const currentDesc = mDescs[i];
                
                // Detection logic matching your EJS labels
                const isUrlType = /URL|קישור|Link|ссылка/i.test(currentType);

                if (isUrlType) {
                    // Grab the next available URL from the filtered array sent by the browser
                    finalPath = mUrls[urlPointer] || '';
                    urlPointer++;
                } else if (files[filePointer]) {
                    // Handle Physical File
                    const file = files[filePointer];
                    const safeFileName = `${Date.now()}-${file.originalname.replace(/\s+/g, '_')}`;
                    const fullDestPath = path.join(targetDir, safeFileName);

                    fs.copyFileSync(file.path, fullDestPath);
                    fs.unlinkSync(file.path);

                    finalPath = `/soldierUploads/${folderName}/${safeFileName}`;
                    filePointer++;
                }

                // 3. Database Insert for this specific row
                if (finalPath || currentDesc) {
                    await t.none(`
                        INSERT INTO soldier_multimedia (soldier_id, file_type, file_path, description)
                        VALUES ($1, $2, $3, $4)
                    `, [soldierId, currentType, finalPath, currentDesc]);
                }
            }
        });
