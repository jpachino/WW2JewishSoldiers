...
app.post('/addFULL', multiUpload, async (req, res) => {
    try {
       ...
       // 2. CATEGORY CHECKBOX LOGIC (cat1 - cat8)
      for (let i = 1; i <= 8; i++) {
        const fieldName = `cat${i}`;
        const fieldNameEn = `cat${i}en`;
        const fieldNameRu = `cat${i}ru`;
        
    
        // If checked, req.body[fieldName] now contains the title in the active language
        // If not checked, we store an empty string
        cleaned[fieldName] = req.body[fieldName] || ''; 
        cleaned[fieldNameEn] = req.body[fieldNameEn] || ''; 
        cleaned[fieldNameRu] = req.body[fieldNameRu] || '';
      }

        // 3. DYNAMIC TRANSLATION LOOKUPS
        // Removed categoryId from here as it's now handled by cat1-cat8
        const translations = [
...
// 1. Force description, types, and locations into arrays
...
// 2. Identify the files array from Multer
// Matches the name in your multiUpload: 'm_files[]'
let mFiles = (req.files && req.files['m_files[]']) ? req.files['m_files[]'] : [];

// Determine how many rows we need to process
const maxRows = Math.max(rawDesc.length, rawTypes.length, rawLocs.length);
let fileIdx = 0; // Tracks the current file in the mFiles array
...
for (let i = 0; i < maxRows; i++) {
    ...
    // Check if the user chose a "Link/URL" type
    const isURL = currentType.includes('קישור') || currentType.toLowerCase().includes('url');

    // 3. Process File if this isn't a URL row and a file exists
    if (!isURL && mFiles[fileIdx]) {
        const file = mFiles[fileIdx];
        
        // Use the filename generated by your storage config (which already decoded UTF8)
        const fileName = file.filename; 
        const finalPath = path.join(targetDir, fileName);
        
        try {
            // Move from /var/data/soldierUploads/temp to /var/data/soldierUploads/A{id}
            fs.renameSync(file.path, finalPath);
            
            // This is the path stored in the DB for the browser to find
            finalDbPath = `/soldierUploads/${folderName}/${fileName}`;
            fileIdx++; 
        } catch (renameErr) {
    ...
    // 4. Database Insert
    // Only insert if there is a file, a URL, or at least a description
    if (finalDbPath || userLocation || rawDesc[i]) {
        ...
        `, [
            ...
            (i === 0 && finalDbPath !== null) // Set first file-based row as profile pic
        ]);
    }
}
console.log(`✅ Successfully processed ${fileIdx} files.`);
}); // Closes Transaction
...
// We loop through 1-8. If it exists in req.body, use it. 
// If not, it's unchecked, so we use an empty string.
...
// --- DIAGNOSTIC LOG ---
console.log('--- Category Update Sync Check ---');
for (let i = 1; i <= 8; i++) {
    if (req.body[`cat${i}`]) {
        console.log(`Row ${i}: HE: ${req.body['cat'+i]}, EN: ${req.body['cat'+i+'en']}, RU: ${req.body['cat'+i+'ru']}`);
    }
}
       // Main input object
         const inputData = {
...
// 3. Multimedia Logic
            ...
            const existingProfilePic = await t.oneOrNone('SELECT id FROM "multimedia_TBL" WHERE soldier_id = $1 AND is_profile_pic = true LIMIT 1', [soldierId]);
            ...
            for (const file of mFiles) {
                if (file.size > MAX_SIZE) {
                    mFiles.forEach(f => { if (fs.existsSync(f.path)) fs.unlinkSync(f.path); });
                    throw new Error(`File ${file.originalname} is too large (Max 1.5MB).`);
                }
            }

            const maxRows = Math.max(descArr.length, typeArr.length, locArr.length);
            for (let i = 0; i < maxRows; i++) {
                let finalDbPath = null;
                let physicalLocation = locArr[i] || null;
                const fileForThisRow = mFiles[i];
                const selectedType = (typeArr[i] || '').toLowerCase();

                if (fileForThisRow) {
                    const fileName = fileForThisRow.originalname.toLowerCase();
                    const isJpg = fileName.endsWith('.jpg') || fileName.endsWith('.jpeg');
                    const isPdf = fileName.endsWith('.pdf');

                    // TYPE MISMATCH CHECK
                    const isPicType = selectedType.includes('תמונה') || selectedType.includes('picture') || selectedType.includes('фото');
                    const isDocType = selectedType.includes('מסמך') || selectedType.includes('document') || selectedType.includes('документ');

                    if (isPicType && !isJpg) {
                        if (fs.existsSync(fileForThisRow.path)) fs.unlinkSync(fileForThisRow.path);
                        throw new Error(`Row ${i+1}: Pictures must be JPG/JPEG.`);
                    }
                    if (isDocType && !isPdf) {
                        if (fs.existsSync(fileForThisRow.path)) fs.unlinkSync(fileForThisRow.path);
                    ...
                    try {
                        const decodedFileName = Buffer.from(fileForThisRow.originalname, 'latin1').toString('utf8');
                        const finalPath = path.join(targetDir, decodedFileName);
                        fs.renameSync(fileForThisRow.path, finalPath);
                        finalDbPath = `/soldierUploads/${folderName}/${decodedFileName}`;
                    } catch (err) {
                        const safeName = fileForThisRow.originalname;
                        fs.copyFileSync(fileForThisRow.path, path.join(targetDir, safeName));
                        fs.unlinkSync(fileForThisRow.path);
                        finalDbPath = `/soldierUploads/${folderName}/${safeName}`;
                    }
                }

                if (finalDbPath || (physicalLocation && physicalLocation.trim() !== '')) {
                    const setToProfile = (!hasProfilePic && finalDbPath !== null);
                    if (setToProfile) hasProfilePic = true;

                    await t.none(`INSERT INTO "multimedia_TBL" (soldier_id, file_description, file_path, physical_logical_location, multimedia_type, is_profile_pic, uploaded_date)
                        VALUES ($1, $2, $3, $4, $5, $6, NOW())`, 
                        [soldierId, descArr[i] || 'No Description', finalDbPath, physicalLocation, typeArr[i] || null, setToProfile]);
                } else if (fileForThisRow && fs.existsSync(fileForThisRow.path)) {
                    fs.unlinkSync(fileForThisRow.path);
                }
            }
        });
