...
app.get('/soldierlistSoldier', async (req, res) => {
  try {
    ...
    const formattedSoldiers = soldiers.map(soldier => ({
      ...soldier._doc,
      date_of_birth: format(new Date(soldier.date_of_birth),'yyyy-mm-dd'),
...
app.post('/addFULL', async (req, res) => {
  ...
  const dob = date_of_borth ? new Date(date_of_birth) : null;
  const dod = date_of_death ? new Date(date_of_death) : null;
  console.log('Parsed Date of Birth:', dob);
  console.log('Parsed Date of Death:', dod);
  try {
    // Create a new record
...
app.get('/search', (req, res) => {
 ...
 app.get('/searchResults', async (req, res) => {
  ...
  app.post('/updateSoldier/:id', async (req, res) => {
    const { id } = req.params;
    const { firstname, lastname, countrycode, servicebranch, format_date_of_birth, rank, date_of_death } = req.body;
    const dob = new formatDateforInput(date_of_birth);
    const dod = date_of_death ? new Date(date_of_death) : null;
