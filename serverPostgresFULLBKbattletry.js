...
// Show soldier details + battle history
app.get('/soldiers/:id', async (req, res) => {
  const soldierId = req.params.id;

  try {
    const soldier = await db.one('SELECT * FROM "SOLDIER_TABLE" WHERE id = $1', [soldierId]);

    const battleHistory = await db.any(`
      SELECT id, year, front, battle, medal, details
      FROM soldier_battle_history
      WHERE soldier_id = $1
      ORDER BY year ASC
    `, [soldierId]);

    res.render('soldiers/show', { soldier, battleHistory });
  } catch (err) {
    console.error('Error fetching soldier:', err);
    res.status(500).send('Error fetching soldier data');
  }
});
// Show form to add battle history for a soldier
app.get('/soldiers/:id/history/add', async (req, res) => {
  const soldierId = req.params.id;

  try {
    const soldier = await db.one('SELECT id, fname, lname FROM "SOLDIER_TABLE" WHERE id = $1', [soldierId]);
    res.render('add-soldier-battle', { soldier });
  } catch (err) {
    console.error('Error loading battle form:', err);
    res.status(500).send('Error loading form');
  }
});
app.post('/soldiers/:id/history/add', async (req, res) => {
  const soldierId = req.params.id;
  let { year, front, battle, medal, details } = req.body;

  // Convert year string to integer safely
  const yearInt = year ? parseInt(year, 10) : null;

  try {
    await db.none(`
      INSERT INTO soldier_battle_history (soldier_id, year, front, battle, medal, details)
      VALUES ($1, $2, $3, $4, $5, $6)
    `, [soldierId, yearInt, front || null, battle || null, medal || null, details || null]);

    res.redirect(`/soldiers/${soldierId}`);
  } catch (err) {
    console.error('Error adding battle history:', err);
    res.status(500).send('Error saving battle history');
  }
});
...
app.post('/updateSoldier/:id', upload.array('files'), async (req, res) => {
  ...
  try {
    ...
    // Extract arrays from req.body
const { battleyear, front, battle, battle_medal, battle_details } = req.body;

if (Array.isArray(battleyear)) {
  const battleInserts = battleyear.map((year, idx) => {
    // Only insert if at least one field has a value
    if (year || front[idx] || battle[idx] || battle_medal[idx] || battle_details[idx]) {
      return db.none(`
        INSERT INTO soldier_battle_history
        (soldier_id, year, front, battle, medal, details)
        VALUES ($1, $2, $3, $4, $5, $6)
      `, [
        soldierId,
        year || null,
        front[idx] || null,
        battle[idx] || null,
        battle_medal[idx] || null,
        battle_details[idx] || null
      ]);
    }
  });
  await Promise.all(battleInserts);
}
