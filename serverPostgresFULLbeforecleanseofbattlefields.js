...
// **NEW LOGIC: Conditional Redirect**
    if (addbattle) {
      console.log(`✅ Record saved. Redirecting to battles page for ID: ${soldierId}`);
      res.redirect(`/soldiers/${soldierId}/battles`);
    } else {
      console.log('✅ Record saved. No battles to add. Redirecting to confirmation.');
      res.redirect('/?saved=true');
    }
...
app.post('/soldiers/:id/battles', async (req, res) => {
  const { id } = req.params;
  const { battleyeararray, front, battle, battle_medal, battle_details } = req.body;
  ...
  try {
    ...
    await db.tx(async t => {
      ...
      for (let i = 0; i < battleyeararray.length; i++) {
        // Skip empty rows.
        if (!battleyeararray[i] && !front[i] && !battle[i]) {
          continue;
        }
  ...
  try {
    ...
    await db.none(
      `UPDATE soldier_battle_history
       ...
       WHERE id = $6`,
      [yearInt, front, battle, medal, details, battleId]
    );
...
app.post('/addNewBattle', async (req, res) => {
  ...
  try {
    ...
    await db.none(`
      INSERT INTO "battle_TBL" (year, front, battle, medal, details)
      VALUES ($1, $2, $3, $4, $5)
    `, [yearInt, front, battle, medal, details]);
