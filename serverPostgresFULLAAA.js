...
app.post('/addFULL', upload.array('files'), async (req, res) => {
  ...
  try {
    ...
    const {
      ...
      other_medal, other_medalen, other_medalru, addbattle
    } = cleaned;
...
// ✅ Add a single battle for a soldier
app.post('/soldiers/:id/battles', async (req, res) => {
  const { id } = req.params;
  const { year, front, battle, medal, details } = req.body;
  ...
  try {
    await db.none(
      ...
      [
        ...
        front && front.trim() !== '' ? front : null,
        battle && battle.trim() !== '' ? battle : null,
        medal && medal.trim() !== '' ? medal : null,
        details && details.trim() !== '' ? details : null
      ]
    );

    console.log(`✅ Added battle for soldier ID ${id}: ${battle}`);
    res.redirect(`/soldiers/${id}/battles`);
  } catch (err) {
    console.error('❌ Error inserting battle:', err);
    res.status(500).send('Server error while adding battle history.');
  }
});

app.get('/battles/:battleId/edit', async (req, res) => {
  const { battleId } = req.params;
  try {
    const battleResult = await db.oneOrNone('SELECT * FROM soldier_battle_history WHERE id = $1', [battleId]);
    if (!battleResult) {
      return res.status(404).send('Battle not found');
    }

    res.render('editBattle', { battle: battleResult });
  } catch (err) {
    console.error('Error fetching battle for edit:', err);
  ...
  try {
    await db.none(
      `UPDATE soldier_battle_history
       SET year=$1, front=$2, battle=$3, medal=$4, details=$5, updated_at=NOW()
       WHERE id=$6`,
      [year, front, battle, medal, details, battleId]
    );
    res.redirect('back'); // or redirect to the soldier page
  } catch (err) {
